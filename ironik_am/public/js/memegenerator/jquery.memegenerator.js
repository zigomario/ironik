(function($){
	...
	var i18n = {
		...
		addTextbox: "Rajoutez du texte ",
		advancedSettings: "Options",
		toolbox: "Toolbox",
		
		optionCapitalLetters: "Lettes majuscules",
	...
	var MemeGenerator = function(element, options){
		...
		this.ui = {
			createControls: function(){
				...
				{
					var toolboxButton = $('<a href="#" class="mg-toolbox-toggle" style="display: none">' + i18n.toolbox + '</a>').appendTo(controls);
					var toolbox = $('<div class="mg-toolbox"  style="display: none"></div>').appendTo(controls).hide();
			...
			bootstrapify: function(){
				...
				controls.find(".mg-textbox").each(function(){
					if(!$(this).hasClass("row"))
					{
						...
						$(this).find(".mg-textbox-size").addClass("form-control").wrap($("<div></div>").addClass("col-md-1"));
						$(this).find(".mg-textbox-border-width").addClass("form-control").wrap($("<div></div>").addClass("col-md-1"));
				...
				{
					controls.find(".mg-advanced-settings-toggle, .mg-toolbox-toggle").addClass("btn btn-primary btn").wrap($("<div></div>").addClass("row row-toolbox")); // BOUTON OPTION
